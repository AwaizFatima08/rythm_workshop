...
## 1. Host tests (`flutter test`, 93 tests, ~20 s)
...
- Real Galaxy A12 (SM-A125F, Android 12, landscape 753×339 dp): a debug build launched without crashes (tested by another session). Gameplay on that phone is still to be checked, see below.

## 4. Owner checklist before release (needs a real phone and a child)

- [ ] Budget phone (the Galaxy A12, once charged): install `releases/v1.0.0-1/*.apk`, check cold start, play 10 minutes; the chimes still land on the beat by ear (no drift) and the belt stays smooth.
