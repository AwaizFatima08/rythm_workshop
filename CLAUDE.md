...
- `lib/settings/game_settings.dart`: the single `ChangeNotifier` (shared_preferences). `lib/widgets/parental_gate.dart`; `lib/widgets/keep_awake.dart` (screen-on during play, via a channel in `MainActivity.kt`).

## Commands
- Host tests: `flutter test` (94 tests: rules, beat clock, layouts, assets, full level play with real drags, screens).
